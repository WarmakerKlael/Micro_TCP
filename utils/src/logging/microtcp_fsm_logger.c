...
static const char *get_colored_fsm_log_tag_string(enum fsm_log_tag _fsm_log_tag)
{
	switch (_fsm_log_tag)
	{
	case FSM_CONNECT: 	return LOG_FSM_CONNECT_COLOR 	TRANSPORT_PROTOCOL_NAME " " STRINGIFY(FSM_CONNECT) 	LOG_DEFAULT_COLOR;
	case FSM_ACCEPT: 	return LOG_FSM_ACCEPT_COLOR 	TRANSPORT_PROTOCOL_NAME " " STRINGIFY(FSM_ACCEPT)	LOG_DEFAULT_COLOR;
	case FSM_SHUTDOWN:	return LOG_FSM_SHUTDOWN_COLOR 	TRANSPORT_PROTOCOL_NAME " " STRINGIFY(FSM_SHUTDOWN) 	LOG_DEFAULT_COLOR;
	case FSM_RECV:		return LOG_FSM_RECV_COLOR 	TRANSPORT_PROTOCOL_NAME " " STRINGIFY(FSM_RECV) 	LOG_DEFAULT_COLOR;
	case FSM_SEND:		return LOG_FSM_SEND_COLOR 	TRANSPORT_PROTOCOL_NAME " " STRINGIFY(FSM_SEND) 	LOG_DEFAULT_COLOR;
	default:		return LOG_DEFAULT_COLOR 	TRANSPORT_PROTOCOL_NAME " " "??FSM??" 			LOG_DEFAULT_COLOR;
	}
}
// clang-format on
